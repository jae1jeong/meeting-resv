...
import type { BookingResponse, BookingParticipantWithUser } from '@/packages/shared/types/api/booking'
...
const mapBookingToResponse = (booking: BookingWithIncludes): BookingResponse => {
  ...
  const participants: BookingParticipantWithUser[] = booking.participants.map((p) => ({
    id: p.id,
...
export async function GET(
  ...
  try {
    ...
    if (!room) {
      return errorResponse('회의실에 접근 권한이 없습니다', 403)
    }
    ...
    const bookings = await prisma.booking.findMany({
      where: {
        roomId: roomId,
        date: {
          gte: parseKSTDate(startDate),
          lte: setToKSTEndOfDay(parseKSTDate(endDate))
        }
      },
      include: {
        room: {
    ...
    const formattedBookings: BookingResponse[] = bookings.map(mapBookingToResponse)
